...
class ConfigurationChangeRequest(BaseModel):
    ...
    روال به این صورت است که درخواست دهنده درخواست را تکمیل می کند
    بعد از آن مدیر مستقیم درخواست را تایید می کند
    بعد از مدیر مستقیم درخواست برای مدیر مربوطه ارسال می شود
    پس از تایید مدیر مربوطه، درخواست برای دبیر کمیته (در صورت اینکه درخواست نیاز به بررسی کمیته داشته باشد) ارسال می گردد.
    بعد از دبیرکمیته و یا مدیر مربوطه، فرم های تسک که اطلاعات آن ها در جدول 
    Task
    وجود دارد برای مجریان ارسال می گردد.
    زمانی که تمامی تسک ها تمام شود، فرآیند درخواست پایان می یابد
    """
...
class RequestExtraInformation(models.Model):
    """ 
        به ازای هر درخواست یک سری اطلاعات تکمیلی داریم
        مثلا اطلاعات انواع مراکز داده در محل تغییر (ارتباطات مخابراتی، شبکه و ...) در این جدول ذخیره می شوند
        دلیل استفاده از این ساختار این است که بدون محدودیت کاربر بتواند فیلدهای جدید به درخواست اضافه کند
        هر کدام از فیلدهایی که در این قسمت قرار می گیرند ماهیت 
        boolean
        دارند یعنی وجود رکورد مربوطه، به معنی فعال بودن در آن درخواست است
        مثلا اگر برای یک درخواست رکوردی داریم که گزینه «ارتباطات مخابراتی» در آن وجود داشته باشد
        یعنی تغییرات دیتاسنتر در آن درخواست شامل ارتباطات مخابراتی هم می شود
    """    
    extra_info = models.ForeignKey(to=ConstValue, on_delete=models.CASCADE, null=False, verbose_name='شناسه اطلاعات تکمیلی')
...
class NotificationLog(BaseModel):
    ...
    email_to = models.CharField(max_length=4000, verbose_name='آدرس ایمیل دریافت کنندگان To', null=True)
    email_cc = models.CharField(max_length=1000, verbose_name='آدرس ایمیل دریافت کنندگان CC', null=True)
    email_bcc = models.CharField(max_length=1000, verbose_name='آدرس ایمیل دریافت کنندگان BCC', null=True)
